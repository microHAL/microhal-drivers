...
class LIS2DH : protected microhal::I2CDevice {
    ...
    enum class TEMP_CFG_REG_BITS : uint8_t { TEMP_EN0 = 1U << 6, TEMP_EN1 = 1U << 7 };
    ...
    enum class Mode { normal = 0, lowPower = 1, highResolution = 2, notAllowed = 23 };
    ...
    enum class HighPassFilterMode { NormalResetReading = 0, ReferenceSignal, Normal, Autoreset };
    ...
    enum class CLICK_CFG_BITS : uint8_t { XS = 1, XD = 1UL << 1, YS = 1UL << 2, YD = 1UL << 3, ZS = 1UL << 4, ZD = 1UL << 5 };
 ...
 public:
    ...
    LIS2DH(microhal::I2C &i2c, PossibleI2CAddress address) : I2CDevice(i2c, address) {
        scale = Scale::Scale2G;
    ...
    enum Scale : uint8_t { Scale2G = 0, Scale4G = 1UL << 4, Scale8G = 2UL << 4, Scale16G = 3UL << 4 };
 ...
 public:
    ...
    microhal::Acceleration convertToAcceleration(int16_t value) {
        ...
        float temp = range * static_cast<float>(value) / static_cast<float>(1 << 15);
    ...
    friend uint8_t operator|(LIS2DH::INT1_CFG_BITS a, LIS2DH::INT1_CFG_BITS b) { return static_cast<uint8_t>(a) | static_cast<uint8_t>(b); }
    ...
    bool setClick(microhal::Acceleration threshold, std::chrono::microseconds timeLimit, std::chrono::microseconds timeLatency,
                  std::chrono::microseconds timeWindow, bool xAxis, bool yAxis, bool zAxis, bool singleClick, bool doubleClick) {
        uint8_t temp = 0;
        ...
        uint32_t rawTimeLatency = getDataRate() * timeLatency.count() / std::micro::den;
        uint32_t rawTimeWindow = getDataRate() * timeWindow.count() / std::micro::den;
